...
class odoo_remote_container:
    def __init__(self,db="dummy",odoo_image="odoo:12.5",odoo_config = None,host_server = None, db_server = None, version = "16.0"):
...
#        return used_ports
    ...
    def login_remote(self):
        try:
            ...
            _logger.error("Couldn't connect remote")
            return False
...
def main(context=None):
    ...
    odoo_version = context.get("version","16.0")
    host_domain = context.get("host_domain")

    if odoo_version not in ["11.0","12.0","13.0","14.0","15.0", "16.0"]:
...
def create_db_template(db_template=None,modules=None, config_path=None,host_server = None, db_server = None, version = "16.0"):
    _logger.info(locals())

    response = {}
    if version not in ["11.0","12.0","13.0","14.0","15.0", "16.0"]:
        raise Exception("Version not valid")
